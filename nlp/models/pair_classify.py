...
@Model.register("pair_classify")
class PairAttention(Model):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._bias = torch.nn.Parameter(torch.tensor(1.0))
    ...
    def forward(  # type: ignore
        self,
        word1: TextFieldTensors,
        word2: TextFieldTensors,
        ...
        """
        embedded_premise = self._text_field_embedder(word1)
        embedded_hypothesis = self._text_field_embedder(word2)
        premise_mask = get_text_field_mask(word1)
        hypothesis_mask = get_text_field_mask(word2)
        ...
        label_relu = 4*torch.nn.Sigmoid()(label_logits.squeeze(-1)) + self._bias
        #label_probs = torch.nn.functional.softmax(label_logits, dim=-1)

        output_dict = {
            "label_logits": label_logits,
            "label_relu": label_relu,
            "h2p_attention": h2p_attention,
            "p2h_attention": p2h_attention,
        }

        if score is not None:
            loss = self._loss(label_relu.type(torch.DoubleTensor),score.type(torch.DoubleTensor))
    ...
    def make_output_human_readable(
        ...
        """
        new_output = {}
        new_output['score'] = output_dict['label_relu']
